...
class Part2Spec extends CatsEffectSuite {
  ...
  test("Joltage semigroup") {
    assertEquals(
      obtained = (Joltage.fromSeq(Vector(0, 1, 1)) |+| Joltage.fromSeq(Vector(1, 0, 1, 1))),
      expected = Joltage.fromSeq(Vector(1, 1, 2, 1))
    )
    assertEquals(
      obtained = (Joltage.fromSeq(Vector(0, 1, 1)) |+| Joltage.empty),
      expected = Joltage.fromSeq(Vector(0, 1, 1))
    )
  }
